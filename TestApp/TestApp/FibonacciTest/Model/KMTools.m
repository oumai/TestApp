...
+ (BOOL)getAuthStatus:(UIViewController *)viewController
{
    if(IS_IOS7)
    {
        ...
        else
        {
            ...
            if (IS_IOS8)
            {
                UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"无法启动相机" message:@"请开启相机权限：设置->隐私->OTC-BAT" preferredStyle:UIAlertControllerStyleAlert];
...
#else
            UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"无法启动相机" message:@"请开启相机权限：设置->隐私->OTC-BAT" delegate:nil cancelButtonTitle:@"取消" otherButtonTitles:nil, nil];
            [alertView show];
#endif
            return isAvalible;
        }
...
//    NSString *url = [NSString stringWithFormat:@"http://itunes.apple.com/cn/lookup?id=%@",@""];
//    
